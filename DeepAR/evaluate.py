...
def save_predictions_to_csv(predictions, targets, test_start, test_end, stride_size, output_file):
    ...
    original_data_path = '../data/stock/cvs_stock_wsenti.csv'  # Adjust to your file path
    ...
    try:
        ...
        logger.info("Continuing with target values from model evaluation")
        df['actual'] = np.nan
        
        # Try to add target values from model evaluation
        if targets.size > 0:
            # Create a mapping from dates to target values
            target_dates = []
            target_values = []
            ...
            # If no original actual, use model's actual
            mask = df['actual'].isna()
