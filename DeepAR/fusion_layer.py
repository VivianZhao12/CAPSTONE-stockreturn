...
class FusionLayer:
    ...
    def run_pipeline(self, daily_predictions_path, quarterly_data_path, future_days=None, test_size=0.2):
        ...
        if future_days is not None:
            ...
            # 合并历史和未来预测
            # 只保留必要的列用于输出
            essential_cols = ['Date', 'prediction', 'fusion_prediction']
            if 'actual' in result_df.columns:
                essential_cols.append('actual')
            
            # 确保两个DataFrame有相同的列
            common_cols = [col for col in essential_cols if col in future_predictions.columns]
            result_output = result_df[common_cols]
            future_output = future_predictions[common_cols]
            
            all_results = pd.concat([result_output, future_output])
            return all_results
...
if __name__ == "__main__":
    ...
    # 保存结果
    output_dir = 'experiments/cvs_base_model'
